...
export default function DashboardLayout({
  ...
  const styles = {
    ...
    topbarTitle: { fontFamily: FONTS.title, fontSize: 18, fontWeight: 700, color: textPri },
    topbarRight: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 13.5, color: textMut, minWidth: 0, flexShrink: 1, overflow: 'hidden' },
    topbarSep:   { color: border, userSelect: 'none' },
    userBadge:   { display: 'flex', alignItems: 'center', gap: 8, paddingLeft: 14, borderLeft: `1px solid ${border}` },
    avatar:      { width: 32, height: 32, borderRadius: '50%', background: avatarGradient, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 13, fontWeight: 700, flexShrink: 0, userSelect: 'none' },
    username:    { fontSize: 13.5, fontWeight: 600, color: textPri, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 'clamp(60px,15vw,160px)' },
    // ✅ CORRECTION : contentZone était manquant → le contenu ne scrollait pas et débordait
    contentZone: { flex: 1, overflowY: 'auto', overflowX: 'hidden', minHeight: 0 },
  };
          ...
          <header style={styles.topbar}>
            ...
            <div style={styles.topbarRight}>
              ...
              <div style={styles.userBadge}>
                <div style={styles.avatar} title={user?.username ?? ''}>{getInitials(user?.username ?? '')}</div>
                <span style={styles.username}>{user?.username ?? '—'}</span>
              </div>
            </div>
          </header>
          {/* ✅ contentZone maintenant correctement référencé */}
